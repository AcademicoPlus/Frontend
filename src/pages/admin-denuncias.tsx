  ...
  return (
    <div className="pb-12 max-w-5xl mx-auto">
      <header className="mb-8">
        <h1 className="text-2xl md:text-3xl font-extrabold text-[#183E6C] dark:text-blue-300 tracking-tight">Denúncias</h1>
        <p className="text-gray-500 dark:text-gray-400 mt-1 font-medium">{denuncias.length} denúncia(s) pendente(s) de revisão</p>
      </header>
      ...
      {carregando ? (
        <div className="flex justify-center py-20"><p className="text-lg font-bold text-gray-400 dark:text-gray-500 animate-pulse">Carregando denúncias...</p></div>
      ) : denuncias.length === 0 ? (
        <div className="bg-white dark:bg-slate-900 rounded-3xl border border-gray-100 dark:border-slate-700 p-12 text-center shadow-sm">
          <p className="text-lg text-gray-500 dark:text-gray-400 font-medium">Nenhuma denúncia pendente.</p>
        </div>
      ) : (
        <div className="flex flex-col gap-6">
          {denuncias.map((denuncia) => (
            <div key={denuncia.id} className="bg-white dark:bg-slate-900 rounded-3xl border border-gray-100 dark:border-slate-700 shadow-sm p-6 md:p-8">
              <div className="flex flex-col md:flex-row justify-between md:items-center gap-2 mb-5">
                <p className="text-sm font-bold text-[#183E6C] dark:text-blue-300">
                  {denuncia.denunciante.nome} denunciou uma avaliação de {denuncia.avaliador.nome}
                </p>
                <span className="text-xs font-semibold text-gray-400 dark:text-gray-500">{formatarData(denuncia.criadoEm)}</span>
              </div>

              <div className="bg-gray-50 dark:bg-slate-800 rounded-xl p-4 border border-gray-100 dark:border-slate-700 mb-4">
                <p className="text-[11px] font-extrabold text-gray-400 dark:text-gray-500 uppercase tracking-widest mb-2">
              ...
              <div className="mb-6">
                <p className="text-[11px] font-extrabold text-gray-400 dark:text-gray-500 uppercase tracking-widest mb-2">Motivo da denúncia</p>
                <p className="text-sm text-gray-700 dark:text-gray-200">{denuncia.motivo}</p>
              </div>

              {erroLinha[denuncia.id] && (
                <div className="mb-4 rounded-xl bg-red-50 dark:bg-red-950/40 border border-red-100 dark:border-red-900/50 px-4 py-3 text-sm text-red-600 dark:text-red-400">
                  {erroLinha[denuncia.id]}
                </div>
              )}
              ...
              <div className="flex flex-col-reverse md:flex-row gap-3 justify-end">
                <button
                  ...
                  className="px-6 py-3 rounded-xl font-bold text-sm text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-slate-800 hover:bg-gray-100 dark:hover:bg-slate-700 transition-colors disabled:opacity-60 disabled:cursor-not-allowed"
                >
                  Marcar improcedente
                </button>
                <button
